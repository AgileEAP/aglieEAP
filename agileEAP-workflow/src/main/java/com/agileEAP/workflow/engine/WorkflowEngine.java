...
public class WorkflowEngine implements IWorkflowEngine {
	 ...
	 */
	@Override
	public IWorkflowPersistence getWorkflowPersistence() {
		return workflowPersistence;
	}

	public void setWorkflowPersistence(IWorkflowPersistence workflowPersistence) {
	...
	@Override
	public final String CreateAProcess(String processDefID,
			String parentProcessDefID, String parentActivityID) {
		...
		if (processDef == null) {
			WorkflowException workflowException = new WorkflowException(
					String.format("ID=%1$s的流程不存在", processDefID));
			WFUtil.HandleException(workflowException);

			return "";
		}

		if (processDef.getCurrentState() == (short) ProcessStatus.Candidate
				.getValue()) {
			WorkflowException workflowException = new WorkflowException(
					String.format("ID=%1$s的流程还未发布，不能创建流程", processDefID));
			WFUtil.HandleException(workflowException);
		...
		java.util.Date startTime = new java.util.Date(
				new java.util.Date().getTime() - 2000);
		...
		processInst.setCurrentState((short) ProcessInstStatus.NoStart
				.getValue());
		...
		activityInst.setCurrentState((short) ActivityInstStatus.Compeleted
				.getValue());
	...
	public final void StartAProcess(String processInstID,
		...
		ProcessInst processInst = GetToRunProcessInst(processInstID,
				WFUtil.getUser());
		ProcessDefine processDefine = workflowPersistence
				.GetProcessDefine(processInst.getProcessDefID());
		ProcessContext processContext = new ProcessContext(processDefine,
				processInst, parameters);

		ProcessRuntime runtime = new ProcessRuntime(workflowPersistence,
				processContext, null);
		runtime.Run();
	}
	...
	private ProcessInst GetToRunProcessInst(String processInstID, ShiroUser user) {
		ProcessInst processInst = workflowPersistence
				.GetProcessInst(processInstID);
		ProcessDefine processDefine = GetProcessDefine(processInst
				.getProcessDefID());
		String processDefID = processInst.getProcessDefID();

		// 如果流程还未启动，产生一个活动实例
		if (processInst.getCurrentState() == (short) ProcessInstStatus.NoStart
				.getValue()) {
			Activity srcActivity = processDefine.getStartActivity();
			ActivityInst srcActInst = workflowPersistence.GetActivityInsts(
					processInstID, srcActivity.getID(),
					ActivityInstStatus.Compeleted).get(0);
			List<Activity> destActivities = workflowPersistence
					.GetOutActivities(processDefID, srcActivity.getID());
			...
			for (Activity destActivity : destActivities) {
				...
				destActInst.setCurrentState((short) ActivityInstStatus.NoStart
						.getValue());
				...
				destActInst.setType((short) destActivity.getActivityType()
						.getValue());
				...
				if (destActivity instanceof ManualActivity) {
					ManualActivity ma = (ManualActivity) ((destActivity instanceof ManualActivity) ? destActivity
							: null);
					...
					workItem.setBizState((short) WorkItemBizStatus.Common
							.getValue());
					workItem.setCreateTime(startTime);
					workItem.setCreator(user.getId());
					workItem.setCreatorName(user.getName());
					workItem.setCurrentState((short) WorkItemStatus.WaitExecute
							.getValue());
					...
					participant.setDelegateType((short) DelegateType.Sponsor
							.getValue());
					participant.setName(user.getName());
					participant.setParticipantID(user.getId());
					participant.setParticipantIndex(1);
					participant
							.setParticipantType((short) ParticipantorType.Person
									.getValue());
					participant.setPartiInType((short) PartiInType.Exe
							.getValue());
				...
				srcActInst
						.setCurrentState((short) ActivityInstStatus.Compeleted
								.getValue());
	...
	public final void SuspendProcessInst(String processInstID) {
		 ...
		 * workflowPersistence.GetProcessInst(processInstID);
		 * repository.<ActivityInst>ExecuteNonQuery(String.format(
		 * "Update WF_ActivityInst Set CurrentState=%1$s Where ProcessInstID='%2$s' and CurrentState=%3$s and RollbackFlag=0"
		 * , (short)ActivityInstStatus.Suspended.getValue(), processInstID,
		 * (short)ActivityInstStatus.Running.getValue()));
		 * repository.<WorkItem>ExecuteNonQuery(String.format(
		 * "Update WF_WorkItem Set CurrentState=%1$s Where ProcessInstID='%2$s' and CurrentState=%3$s"
		 * , (short)WorkItemStatus.Suspended.getValue(), processInstID,
		 * (short)WorkItemStatus.WaitExecute.getValue()));
		 * processInst.CurrentState =
		 * (short)ProcessInstStatus.Suspended.getValue();
		 * repository.SaveOrUpdate(processInst);
		 */

		workflowPersistence.UpdateActivityInstStatus(processInstID,
				ActivityInstStatus.Running, ActivityInstStatus.Suspended);
		workflowPersistence.UpdateWorkItemStatus(processInstID,
				WorkItemStatus.WaitExecute, WorkItemStatus.Suspended);
		workflowPersistence.UpdateProcessInstStatus(processInstID,
				ProcessInstStatus.Suspended);
	}
	...
	public final void ResumeProcessInst(String processInstID) {
		workflowPersistence.UpdateActivityInstStatus(processInstID,
				ActivityInstStatus.Suspended, ActivityInstStatus.Running);
		workflowPersistence.UpdateWorkItemStatus(processInstID,
				WorkItemStatus.Suspended, WorkItemStatus.WaitExecute);
		workflowPersistence.UpdateProcessInstStatus(processInstID,
				ProcessInstStatus.Running);
	}
	...
	public final void TerminateProcessInst(String processInstID) {
		workflowPersistence.UpdateActivityInstStatus(processInstID,
				ActivityInstStatus.Running, ActivityInstStatus.Terminated);
		workflowPersistence.UpdateWorkItemStatus(processInstID,
				WorkItemStatus.WaitExecute, WorkItemStatus.Terminated);
		workflowPersistence.UpdateProcessInstStatus(processInstID,
				ProcessInstStatus.Terminated);
	}
	...
	public final void CompleteActivityInst(String activityInstID) {
		...
		// (wi.CurrentState ==WorkItemStatus.WaitExecute || wi.CurrentState
		...
		for (WorkItem wi : workflowPersistence.GetWorkItems(activityInstID)) {
			if (wi.getCurrentState() == (short) WorkItemStatus.WaitExecute
					.getValue()
					|| wi.getCurrentState() == (short) WorkItemStatus.Suspended
							.getValue()) {
	...
	public final void ResetActivityInst(String activityInstID) {
		...
		// {
		// foreach (var wi in repository.Query<WorkItem>().Where(wi =>
		// wi.ActivityInstID == activityInstID).ToArray())
		// {
	...
	public final void SuspendActivityInst(String activityInstID) {
		...
		workflowPersistence.UpdateActivityInstStatus(activityInstID,
				ActivityInstStatus.Suspended);
	}
	 ...
	 */
	@Override
	public final PageDataResult GetMyWorkItems(String userId,
			Map<String, Object> parameters, String orderby,
			RequestPageData pageInfo, boolean includeAuto) {
		return workflowPersistence.GetMyWorkItems(userId, parameters, orderby,
				pageInfo, includeAuto);
	}
	 ...
	 */
	@Override
	public final void CompleteWorkItem(String workItemID,
			Map<String, Object> parameters, IAssignParticipant assignParticipant) {
		WorkItem wi = workflowPersistence.GetWorkItem(workItemID);
		if (wi.getCurrentState() != (short) WorkItemStatus.WaitExecute
				.getValue()
				&& wi.getCurrentState() != (short) WorkItemStatus.Suspended
						.getValue()) {
			WorkflowException workflowException = new WorkflowException(String
					.format("ID=%1$s的工作项%2$s当前状态=%3$s,不能被执行", wi.getId(),
							wi.getName(),
							WorkItemStatus.forValue(wi.getCurrentState()))
					.toString());
			WFUtil.HandleException(workflowException);
			return;
		}
		...
		workflowPersistence.UpdateWorkItem(wi);
		workflowPersistence.UpdateActivityInstStatus(wi.getActivityInstID(),
				ActivityInstStatus.Running);

		ProcessInst processInst = workflowPersistence.GetProcessInst(wi
				.getProcessInstID());
		ProcessDefine processDefine = workflowPersistence
				.GetProcessDefine(processInst.getProcessDefID());
		ProcessContext processContext = new ProcessContext(processDefine,
		...
		ProcessRuntime runtime = new ProcessRuntime(workflowPersistence,
				processContext, assignParticipant);
		runtime.Run();
	}
	 ...
	 */
	@Override
	public final void DelegateWorkItem(ShiroUser user, String agentID,
			String workItemID) {
	...
	public final void FetchWorkItem(ShiroUser user, String workItemID) {
		workflowPersistence.UpdateWorkItemStatus(workItemID,
	 ...
	 */
	@Override
	public final Map<WorkItem, List<Operator>> GetActiveWorkItems(
			String processInstID) {
		Map<WorkItem, List<Operator>> result = new HashMap<WorkItem, List<Operator>>();

		Map<String, Object> parameters = new HashMap<String, Object>();
		parameters.put("processInstID", processInstID);
		parameters.put("currentState",
				(short) WorkItemStatus.WaitExecute.getValue());

		List<WorkItem> activeWorkItems = workflowPersistence
				.getWorkItems(parameters);
		for (WorkItem item : activeWorkItems) {
			List<Operator> participants = workflowPersistence
					.GetWorkItemParticipant(item.getId());
	...
	/**
	 * 退回工作项 说明：将当前工作项退回到，最近一个可退回的活动。 最近可退回活动:最近可退回活动，是当前活动的主支上最近的一个共同祖先结点。
	 * 具体步骤： 1、寻找最近的一个可退回目标活动。 2、将流程回退到该目标活动。
	 * 
	 * @param user
	 *            当前用户
	 * @param workItemID
	...
	public final void RollbackWorkItem(ShiroUser user, String workItemID) {
		...
		if (wi.getCurrentState() != (short) WorkItemStatus.Executing.getValue()) {
			ActivityInst actInst = workflowPersistence.GetActivityInst(wi
					.getActivityInstID());
			// 寻找最近的一个可退回活动。
			Activity destActivity = GetFirstRollbackActivity(wi.getProcessID(),
					actInst.getActivityDefID());

			if (destActivity.getActivityType() != ActivityType.StartActivity) {
				logger.info(String.format("回退工作项%1$s-%2$s到活动%3$s-%4$s",
						wi.getId(), wi.getName(), destActivity.getID(),
						destActivity.getName()));
				// 回退到目标活动
		...
		WorkflowException workflowException = new WorkflowException(
				String.format("ID=%1$s的工作项%2$s当前状态=%3$s,不能被执行", wi.getId(),
						wi.getName(),
						WorkItemStatus.forValue(wi.getCurrentState())));
		WFUtil.HandleException(workflowException);
	}
	 ...
	 * @return
	 */
	private Activity GetForefatherSplitActivity(String processDefID,
			Activity activity) {
		List<Activity> inActivities = workflowPersistence.GetInActivities(
				processDefID, activity.getID());

		if (inActivities.size() > 1
				|| (inActivities.size() == 1 && inActivities.get(0)
						.getActivityType() == ActivityType.StartActivity)) {
			return activity;
		}
	 ...
	 * @param processDefine
	 *            流程定义
	 * @param splitActivites
	 *            分支结点列表
	 * @return
	 */
	private Activity GetFirstRollbackActivity(String processDefID,
			String activityID) {
		List<Activity> inActivities = workflowPersistence.GetInActivities(
	 ...
	 * @return
	 */
	private Activity GetForefatherActivity(String processDefID,
			List<Activity> splitActivites) {
		if (splitActivites != null && splitActivites.size() == 1) {
			return splitActivites.get(0);
		}

		int count = splitActivites.size();
		Activity splitParent1 = GetForefatherSplitActivity(processDefID,
				splitActivites.get(count - 1));
		Activity splitParent2 = GetForefatherSplitActivity(processDefID,
				splitActivites.get(count - 2));
		Activity joinParent = null;
		if (splitParent1.getID().equals(splitParent2.getID())
				&& splitActivites.size() == 2) {
			return splitParent1;
		} else if (!splitParent1.getID().equals(splitParent2.getID())) {
			List<Activity> children = new java.util.ArrayList<Activity>(
					java.util.Arrays.asList(new Activity[] { splitParent1,
							splitParent2 }));
	 ...
	 */
	@Override
	public final List<Activity> GetRollbackActivityList(String processDefID,
			String activityID) {
		...
		if (inActivities.size() == 1) {
			if (inActivities.get(0).getActivityType() != ActivityType.StartActivity) {
				...
				result.addAll(GetRollbackActivityList(processDefID,
						inActivities.get(0).getID()));
			}
			return result;
		}
	 ...
	 *            目标活动
	 */
	private void TerminateSelfAndOutActivity(String processInstID,
			Activity destActivity) {
		ActivityInst destActInst = workflowPersistence.GetActivityInsts(
				processInstID, destActivity.getID()).get(0);

		if (destActInst != null) {
			// var outActInsts =
			// repository.<ActivityInst>FindAll(ParameterBuilder.BuildParameters().SafeAdd("ID",
			// new
			// Condition(String.format("ID in(select DestActInstID from WF_Transition where SrcActInstID='%1$s')",
			// destActInst.ID))).SafeAdd("RollbackFlag", 0));
			Map<String, Object> parameters = new HashMap<String, Object>();
			parameters.put("rollbackFlag", (short) 0);
			parameters
					.put("_rawsql",
							String.format(
									"ID in(select DestActInstID from WF_Transition where SrcActInstID='%1$s')",
									destActInst.getId()));
			List<ActivityInst> outActInsts = workflowPersistence
					.ActivityInsts(parameters);
			...
			if (outActInsts != null) {
				for (ActivityInst actInst : outActInsts) {
					List<WorkItem> workItems = workflowPersistence
							.GetWorkItems(actInst.getId());
					for (WorkItem wi : workItems) {
						wi.setCurrentState((short) WorkItemStatus.Terminated
								.getValue());
						logger.info(String.format("终止工作项%1$s-%2$s", wi.getId(),
								wi.getName()));
						workflowPersistence.UpdateWorkItem(wi);
					}
					actInst.setCurrentState((short) ActivityInstStatus.Terminated
							.getValue());
					actInst.setRollbackFlag((short) 1);
					logger.info(String.format("终止活动实例%1$s-%2$s",
							actInst.getId(), actInst.getName()));

					workflowPersistence.UpdateActivityInst(actInst);
					ProcessInst processInst = workflowPersistence
							.GetProcessInst(processInstID);
					Activity newDestAct = workflowPersistence.GetActivity(
							processInst.getProcessDefID(),
							actInst.getActivityDefID());
					TerminateSelfAndOutActivity(processInstID, newDestAct);
				}
			}
			destActInst.setRollbackFlag((short) 1);
			destActInst.setCurrentState((short) ActivityInstStatus.Terminated
					.getValue());
			logger.info(String.format("终止活动实例%1$s-%2$s", destActInst.getId(),
					destActInst.getName()));
	...
	/**
	 * 回退流程到某个活动 说明：将当前活动实例回退到，某个目标活动。 具体步骤： 1、寻找目标活动实例。
	 * 2、将目标活动及后续已完成的活动实例及工作项终止。 3、终止当前活动实例及工作项。（注：为记录当前执行人再终止一次） 4、重新启动可退回活动。
	 * 
	 * @param user
	 * @param srcActInst
	 * @param destActivity
	 */
	@Override
	public final void RollbackActivity(ShiroUser user, ActivityInst srcActInst,
			Activity destActivity) {
		ActivityInst destActInst = workflowPersistence.GetActivityInsts(
				srcActInst.getProcessInstID(), destActivity.getID()).get(0);
		...
		if (destActInst != null) {
			...
			// 将目标活动实例状态置为未开始
			TerminateSelfAndOutActivity(destActInst.getProcessInstID(),
					destActivity);
			List<WorkItem> workItems = workflowPersistence
					.GetWorkItems(srcActInst.getId());
			for (WorkItem wi : workItems) {
				wi.setCurrentState((short) WorkItemStatus.Terminated.getValue());
			...
			srcActInst.setCurrentState((short) ActivityInstStatus.Terminated
					.getValue());
			srcActInst.setRollbackFlag((short) 1);
			srcActInst.setEndTime(createTime);

			logger.info(String.format("终止活动实例%1$s-%2$s", srcActInst.getId(),
					srcActInst.getName()));
			workflowPersistence.SaveOrUpdateActivityInst(srcActInst);
			workItems = workflowPersistence.GetWorkItems(destActInst.getId());
			for (WorkItem wi : workItems) {
				wi.setCurrentState((short) WorkItemStatus.WaitExecute
						.getValue());
				logger.info(String.format("启动工作项%1$s-%2$s", wi.getId(),
						wi.getName()));
				workflowPersistence.UpdateWorkItem(wi);
			}
			destActInst.setCurrentState((short) ActivityInstStatus.NoStart
			...
			logger.info(String.format("启动活动实例%1$s-%2$s", destActInst.getId(),
					destActInst.getName()));
			workflowPersistence.UpdateActivityInst(destActInst);

			ProcessInst processInst = workflowPersistence
					.GetProcessInst(srcActInst.getProcessInstID());
			Transition transition = new Transition();
			transition.setId(Identities.uuid());
			transition.setDestActID(destActivity.getID());
			transition.setDestActInstID(destActInst.getId());
	...
	@Override
	public final List<Activity> GetSkipActivityList(ShiroUser user,
			Activity srcAct) {
		throw new WorkflowException("NotImplementedException");
	}

	@Override
	public final void SkipToActivity(ShiroUser user, ActivityInst srcActInst,
			Activity destActivity) {
		throw new WorkflowException("NotImplementedException");
	}
	 ...
	 */
	@Override
	public final List<Participantor> GetPersonParticipantors(
			ParticipantorType parentType, String parentID) {
		return participantorService.getPersonParticipantors(parentType,
				parentID);
	}
}
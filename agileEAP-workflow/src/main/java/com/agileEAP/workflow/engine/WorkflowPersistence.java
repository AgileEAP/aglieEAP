...
public class WorkflowPersistence implements IWorkflowPersistence {
	private final Logger logger = LoggerFactory
			.getLogger(WorkflowPersistence.class);
	...
	public ProcessDefine GetProcessDefine(String processDefID) {
		...
		return ProcessDefine.parseFromJson(processDef.getContent());
	}
	 ...
	 */
	@Override
	public Activity GetActivity(String processDefID, String activityDefID) {
		return WFUtil.getActivity(GetProcessDefine(processDefID)
				.getActivities(), activityDefID);
	}
	 ...
	 */
	@Override
	public java.util.List<Activity> GetActivities(String processDefID) {
	 ...
	 */
	@Override
	public List<Activity> GetInActivities(String processDefID, String activityID) {
		...
		for (Activity activity : processDefine.getActivities()) {
			for (com.agileEAP.workflow.definition.Transition transition : processDefine
					.getTransitions()) {
				if (transition.getDestActivity().equals(activityID)
						&& transition.getSrcActivity().equals(activity.getID())) {
	 ...
	 */
	@Override
	public java.util.List<Activity> GetOutActivities(String processDefID,
			String activityID) {
		ProcessDefine processDefine = GetProcessDefine(processDefID);
		...
		for (Activity activity : processDefine.getActivities()) {
			for (com.agileEAP.workflow.definition.Transition transition : processDefine
					.getTransitions()) {
				if (transition.getSrcActivity().equals(activityID)
						&& transition.getDestActivity()
	 ...
	 */
	@Override
	public com.agileEAP.workflow.definition.Transition GetTransition(
			String processDefID, String srcActivityID, String destActivityID) {
		for (com.agileEAP.workflow.definition.Transition transition : GetProcessDefine(
				processDefID).getTransitions()) {
			if (transition.getSrcActivity().equals(srcActivityID)
					&& transition.getDestActivity().equals(destActivityID)) {
				return transition;
			}
		}
	 ...
	 */
	@Override
	public List<com.agileEAP.workflow.definition.Transition> GetInTransitions(
			String processDefID, String activityDefID) {
	 ...
	 */
	@Override
	public List<com.agileEAP.workflow.definition.Transition> GetOutTransitions(
			String processDefID, String activityDefID) {
	 ...
	 */
	@Override
	public List<ActivityInst> GetActivityInsts(String processInstID,
	...
	@Override
	public List<ActivityInst> GetActivityInsts(String processInstID,
			String activityDefID, ActivityInstStatus actInstStatus) {
	 ...
	 */
	@Override
	public java.util.List<ActivityInst> GetActivityInsts(String processInstID) {
		Map<String, Object> parameters = new HashMap<String, Object>();
		parameters.put("processInstID", processInstID);
	 ...
	 */
	@Override
	public java.util.List<WorkItem> GetWorkItems(String activityInstID) {
		Map<String, Object> parameters = new HashMap<String, Object>();
	 ...
	 */
	@Override
	public PageDataResult GetMyWorkItems(String userId,
			java.util.Map<String, Object> parameters, String orderby,
		...
		pageDataResult
				.setTotal(workItemRepository.countMyWorkItems(parameters));
		pageDataResult.setData(workItemRepository
				.getMyWorkItemsByPage(parameters));
		 ...
		 * 
		 * DatabaseType databaseType = UnitOfWork.CurrentDatabaseType; if
		 * (databaseType == DatabaseType.MySQL) {
		 * 
		 * System.Data.DataTable dt =
		 * repository.<WorkItem>ExecuteDataTable(String
		 * .format("select a.* from WF_WorkItem a " + "\r\n" +
		 ...
		 * + "\r\n" +
		 * " 		or p.ParticipantID in (select OrgID from OM_EmployeeOrg o where o.EmployeeID='%1$s')) %2$s) and 1=1 "
		 * , userID, includesql), parameters, (sortCommand != null) ?
		 * sortCommand : "order by CreateTime desc", pageInfo);
		 * 
		 * 
		 * PageOfList<WorkItem> tempVar = new PageOfList<WorkItem>();
		 * tempVar.ItemList = dt.<WorkItem>ToList(); tempVar.PageInfo =
		 * pageInfo; return tempVar; } else { parameters.SafeAdd(userID, new
		 * Condition(String.format("(ID in" + "\r\n" +
		 ...
		 * + "\r\n" +
		 * "                                 exists(select ID from OM_EmployeeOrg o where o.EmployeeID='%1$s' and o.OrgID=p.ParticipantID)) %2$s)"
		 * , userID, includesql)));
		 * 
		 * 
		 * return repository.<WorkItem>FindAll(parameters, (sortCommand != null)
		 * ? sortCommand : "order by CreateTime desc", pageInfo); }
		 */
	}
	 ...
	 */
	@Override
	public void UpdateActivityInstStatus(String activityInstID,
			ActivityInstStatus status) {
	 ...
	 */
	@Override
	public void UpdateProcessInstStatus(String processInstID,
			ProcessInstStatus status) {
	...
	public String GetParticipant(String processDefID, String activityDefID) {
		return "";
		// ManualActivity ma = GetActivity(processDefID, activityDefID) as
		// ManualActivity;

		// //if
		// (ma.Participant.ParticipantType.Cast<ParticipantType>(ParticipantType.Person)
		// == ParticipantType.Person)
		// //{

		// //}
		// return string.Join(",", ma.Participant.Participantors.Select(p =>
		// p.ID).ToArray());
	}
	...
	@Override
	public void UpdateActivityInst(ActivityInst activityInst) {
		activityInstRepository.update(activityInst);
	}
	...
	@Override
	public void SaveActivityInst(ActivityInst activityInst) {
	...
	@Override
	public void SaveTransControl(TransControl transControl) {
		transControlRepository.save(transControl);
	}

	@Override
	public void SaveOrUpdateActivityInst(ActivityInst activityInst) {
	...
	@Override
	public void SaveProcessInst(ProcessInst processInst) {
		processInstRepository.save(processInst);
	}

	@Override
	public void SaveProcessDef(ProcessDef processDef) {
		processDefRepository.save(processDef);
	}

	@Override
	public void UpdateProcessInst(ProcessInst processInst) {
		processInstRepository.update(processInst);
	}

	@Override
	public List<WorkItem> getWorkItems(Map<String, Object> parameters) {
		return workItemRepository.search(parameters);
	}

	@Override
	public void UpdateWorkItemStatus(String processInstID,
	...
	@Override
	public void UpdateActivityInstStatus(String processInstID,
	...
	@Override
	public List<Operator> GetWorkItemParticipant(String workItemID) {
	 ...
	 */
	@Override
	public List<ActivityInst> ActivityInsts(Map<String, Object> parameters) {
		return activityInstRepository.search(parameters);
	}
	 ...
	 */
	@Override
	public List<TransitionModel> GetProcessInstTransitions(String processInstID) {
